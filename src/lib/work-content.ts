...
interface GuidePanel {
  ...
  /** Grouped example visitor questions, rendered as chat-style bubbles —
   * a way to visualize "why people actually come to this screen" without
   * inventing precise usage numbers the research doesn't back up. */
  bubbles?: {
    label: Copy;
    items: Copy[];
  }[];
...
export const WORK_PROJECTS: WorkProject[] = [
  ...
  },
  {
    ...
    description: {
      ko: "한국예술종합학교 학사행정시스템 UI/UX 리디자인 프로젝트입니다. IA 설계부터 레이아웃까지 진행했습니다.",
      en: "A UI/UX redesign for Korea National University of Arts' academic administration portal — from IA design through layout.",
    },
      ...
      panels: [
        {
          ...
          label: { ko: "IA", en: "IA" },
          intro: {
            ko: "실제 화면에서 확인 가능한 범위까지만 정리했습니다 — 메인 사이드바 6개 메뉴와, 학적관리 화면 안에서 다시 갈라지는 상단 탭 7개로 구성됩니다.",
            en: "Organized only as far as the shipped screens actually confirm — a 6-item primary sidebar, and a further 7-tab sub-navigation inside Academic Records.",
          },
          tree: [
            {
        ...
        },
        {
          ...
          ranking: [
            ...
            {
              label: { ko: "수강신청", en: "Course Registration" },
              note: {
                ko: "학기 초 접속이 몰리는 기능",
          ...
          resolution: {
            ko: "그래서 이용 빈도가 가장 높은 학적·수강신청을 메인 화면 최상단에 노출하고, 사이드바를 6개 카테고리로 압축해 탐색 경로를 줄였습니다. 학적관리처럼 정보량이 많은 화면은 좌측 요약 카드·우측 입력 폼으로 나눠, 훑어보는 흐름과 실제로 조작하는 영역을 분리했습니다.",
            en: "So I surfaced the highest-frequency actions — Academic Records, Course Registration — right at the top of the main dashboard, and compressed the sidebar to 6 categories to shorten the path to them. For information-dense screens like Academic Records, I split the layout into a left summary card and a right input form, separating what you scan from what you act on.",
          },
        },
      ],
    },
  },
  {
    ...
    process: {
      ko: "조약은 자료마다 필요한 정보가 전부 다릅니다. 세 언어를 나란히 대조해야 하는 조약이 있고, 다른 조약과 조문을 비교해야 하는 조약이 있으며, 수백 페이지 스캔 원문을 직접 넘겨가며 읽어야 하는 조약도 있습니다. 화려한 연출보다 정보의 위계와 신뢰감이 우선인 정부기관 서비스에서, 이 모든 경우를 하나의 흐름 안에 담는 것이 과제였습니다. 정보구조 설계부터 검색 결과 화면 기획·디자인, 원문 뷰어 설계, 반응형 대응까지 진행했습니다.",
      en: "Every treaty needs something different. Some need three languages compared side by side, some need their articles cross-referenced against another treaty, and some are hundreds of pages of scanned originals that have to be paged through directly. For a government service where information hierarchy and a sense of trust matter more than visual flourish, the challenge was fitting every one of these cases into a single coherent flow. I handled information architecture, the search results screen, the document viewer, and responsive design.",
    },
    ...
    designGuide: {
      panels: [
        ...
        },
        {
          ...
          list: [
            {
              label: { ko: "한 면 / 두 면 보기 전환", en: "Single / spread page toggle" },
              note: {
            ...
            {
              label: { ko: "페이지 회전", en: "Page rotation" },
              note: {
            ...
            {
              label: { ko: "화면 배율 및 전체화면", en: "Zoom & fullscreen" },
              note: {
            ...
            {
              label: { ko: "배경색 전환", en: "Background toggle" },
              note: {
            ...
            {
              label: { ko: "목차 보기", en: "Table of contents" },
              note: {
            ...
            {
              label: { ko: "자동 넘김", en: "Auto page-turn" },
              note: {
            ...
            {
              label: { ko: "국문/영문 전환, 서명 위치로 이동", en: "KO/EN switch, jump to signature page" },
              note: {
  ...
  },
  {
    ...
    description: {
      ko: "국립중앙박물관 대표 홈페이지의 메인 화면과 정보 구조를 개선한 리뉴얼 시안입니다.",
      en: "A renewal proposal improving the National Museum of Korea's main site — its home screen and information architecture.",
    },
    process: {
      ko: "사람들이 이 사이트에 오는 이유는 대체로 두 가지입니다. 관람시간·관람료 같은 방문 정보를 확인하려고, 그리고 지금 어떤 전시와 교육이 열리고 있는지 보려고. 기존 화면에서는 이 두 가지 정보가 여러 메뉴에 흩어져 있어, 원하는 답을 얻기까지 여러 번의 이동이 필요했습니다. 박물관 홈페이지는 정보는 명확하게 전달하되, 기관이 다루는 유물과 전시가 가진 미감도 함께 담아야 합니다. 정보만 남기면 관공서 게시판이 되고, 비주얼만 좇으면 필요한 정보를 찾을 수 없습니다. 이 균형을 잡는 것이 과제였습니다. 벤치마킹 분석부터 정보구조 개선, 메인 화면과 GNB 디자인까지 진행했습니다. 국내외 6개 기관을 분석해 공통 패턴을 찾고, 그 근거 위에서 GNB를 재편하고, 사용자가 가장 자주 찾는 정보를 가장 먼저 보이는 자리로 옮긴 작업입니다. 정보 접근성과 시각적 완성도는 종종 서로 상충하는 것처럼 보이지만, 무엇을 덜어낼지 결정하고 나면 두 가지가 같은 방향을 향한다는 것을 확인했습니다.",
      en: "There are, broadly, two reasons people come to this site: to check visit info like hours and admission, and to see what exhibitions and programs are currently running. On the existing site, both were scattered across separate menus, so getting either answer took several trips through the navigation. A museum's homepage has to deliver information clearly while still carrying the sense of craft in what the institution actually holds — lean only on information and it reads like a government bulletin board; chase only visuals and the information people need goes missing. Striking that balance was the job. I ran the benchmarking analysis, reworked the information architecture, and designed the main screen and GNB — analyzing six institutions for a shared pattern, rebuilding the GNB on that evidence, and moving the information visitors look for most into the first place they'd see it. Access to information and visual polish often look like they pull in opposite directions, but once you decide what to cut, it turns out they point the same way.",
    },
    ...
    designGuide: {
      panels: [
        ...
        },
        {
          ...
          intro: {
            ko: "기존 메뉴는 기관의 조직 구조를 반영해 여러 갈래로 나뉘어 있었습니다. 이를 '이용자가 무엇을 하러 왔는가'를 기준으로 재편했습니다 — 관람정보·전시·문화·교육·행사·소장품·학술행사·출판·박물관이야기, 6개입니다. 각 대메뉴는 클릭 시 하위 항목이 좌우 2단으로 펼쳐지며, 왼쪽에서 중분류를 고르면 오른쪽에 소분류가 나타납니다.",
            en: "The existing menu was split into several branches that mirrored the institution's own org chart. I rebuilt it around what a visitor actually came to do instead — Visit Info, Exhibitions & Culture, Education & Events, Collection, Academic & Publications, Museum Stories: six items. Each opens into a two-column dropdown — pick a mid-level category on the left, and its sub-items appear on the right.",
          },
          tree: [
            {
        ...
        },
        {
          ...
          bubbles: [
            {
              label: { ko: "방문 정보가 궁금해요", en: "I need visit info" },
              items: [
                { ko: "오늘 몇 시까지 해요?", en: "What time does it close today?" },
                { ko: "관람료는 얼마예요?", en: "How much is admission?" },
                { ko: "예약이 필요한가요?", en: "Do I need to book ahead?" },
              ],
            },
            {
              label: { ko: "지금 뭐 하는지 궁금해요", en: "I want to know what's on" },
              items: [
                { ko: "요즘 무슨 전시 해요?", en: "What's showing right now?" },
                {
                  ko: "이번 주말에 아이랑 갈만한 프로그램 있어요?",
                  en: "Anything good to bring the kids to this weekend?",
                },
              ],
            },
          ],
